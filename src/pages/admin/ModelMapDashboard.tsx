...
export default function ModelMapDashboard() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-rajdhani font-bold neon-text-cyan">My Model Map</h1>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <TabsTrigger
                key={cat.id}
                value={cat.id}
                className="data-[state=active]:bg-cyan-500/20 data-[state=active]:text-[hsl(var(--color-cyan))] data-[state=active]:border-cyan-500 border border-transparent px-4 py-2 rounded-md transition-all"
                style={{
                  background: 'rgba(26, 11, 46, 0.4)',
                }}
              >
          ...
          <TabsContent key={cat.id} value={cat.id} className="mt-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  {winnerModel ? (
                    ...
                    <p className="text-sm text-[hsl(var(--color-light-text))] opacity-50 italic">
                      No winner determined yet. Complete more tests!
                    </p>
                  )}
              ...
              >
                ...
                {currentInsight?.pro_tip ? (
                  ...
                  <p className="text-sm text-[hsl(var(--color-light-text))] opacity-50 italic">
                    Complete more tests to generate AI-powered insights.
                  </p>
                )}
              </Card>
            </div>
            ...
            {modelAverages.length > 0 && (
              ...
              >
                ...
                {/* Legend */}
                <div className="flex gap-4 mt-4 pt-4 border-t border-cyan-500/20">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded bg-green-500/30"></div>
                    <span className="text-xs text-green-400">Strong (4-5)</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded bg-orange-500/30"></div>
                    <span className="text-xs text-orange-400">Moderate (2.5-4)</span>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            {modelAverages.length === 0 && (
              ...
              >
                ...
                <h3 className="text-xl font-rajdhani font-bold text-[hsl(var(--color-cyan))] mb-2">
                  No Data for {cat.label}
                </h3>
                <p className="text-[hsl(var(--color-light-text))] opacity-70">
                  Complete tests with {cat.label.toLowerCase()} prompts to populate this dashboard.
                </p>
              </Card>
            )}
          </TabsContent>
